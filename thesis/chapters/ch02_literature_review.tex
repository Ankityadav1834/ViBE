...
A key architectural strength of liionpack is its \textbf{positional busbar model}: each busbar segment is an independent resistor in the netlist. This captures the asymmetry between cells near the terminal and cells at the far end of the busbar row, which can cause current non-uniformity of up to 20\% in wide parallel strings~\cite{Tranter2022}. However, as models scale above 20 cells, computational complexity in traditional CPU-bound solvers tends to increase exponentially, making exact, fully-coupled nonlinear physics simulations extremely slow. This exponential scaling severely restricts most pack models to either purely series strings, purely parallel strings, or heavily reduced-order physics.
...
\section{Sensitivity Analysis of Electrochemical Models}
\label{sec:lit_sensitivity}

Prior work on electrochemical battery sensitivity analysis has primarily relied on perturbation-based or repeated-simulation approaches because DFN-type electrochemical models are computationally expensive and strongly nonlinear. Several studies perform sensitivity analysis by repeatedly perturbing model parameters and re-running the electrochemical solver to observe changes in outputs such as voltage response, thermal behavior, or parameter identifiability~\cite{ParamSens}.

For example, approaches that estimate parameters of electrochemistry-based lithium-ion battery models using a two-step procedure typically perform sensitivity analysis of DFN parameters to identify which electrochemical parameters can be reliably estimated from current-voltage measurements~\cite{ParamEst}. Their approach evaluates parameter importance through repeated DFN simulations under perturbed parameter sets.

Similarly, parameter estimation studies of the Doyle--Fuller--Newman model often use parameter normalization, grouping, and sensitivity analysis to reduce parameter dimensionality and improve parameter estimation robustness~\cite{SurrogateAging}. However, the methodology still relies on repeated forward DFN evaluations for parameter perturbations.

Other works have also highlighted the computational burden of DFN sensitivity studies. The computational complexity of DFN models limits their use for large-scale analysis and motivates reduced-order approximations~\cite{Jokar2016}. Model simplifications and their impact on computational complexity have explicitly studied the tradeoff between model accuracy and simulation time~\cite{Moura2017}.

Recent global sensitivity studies of the DFN model also emphasize the difficulty of performing sensitivity analysis on highly nonlinear time-dependent electrochemical systems. Many existing methods become computationally challenging for time-dependent PDE outputs because the DFN model contains strongly coupled nonlinear electrochemical dynamics.

\subsection{Limitations of Perturbation Methods}
\label{ssec:lit_perturbation_limits}

The primary limitation of these traditional approaches is that computational cost scales linearly with the number of parameters. For a central finite-difference sensitivity study with $N_p$ parameters, $2N_p + 1$ full nonlinear simulations are typically required (one baseline simulation, one positive perturbation, and one negative perturbation for each parameter).

For coupled electrochemical-aging-thermal pack models, this rapidly becomes computationally prohibitive. 

\subsection{Differentiable Electrochemical Physics}
\label{ssec:lit_differentiable_physics}

Recent differentiable battery modeling work, such as \emph{DiffLiB}~\cite{Wang2024}, highlights the growing importance of differentiable electrochemical simulation frameworks for efficient parameter identification and gradient-based optimization. DiffLiB demonstrated high-fidelity differentiable modeling of lithium-ion batteries.

However, the proposed framework in this thesis extends these concepts toward tensorized pack-level electrochemical-aging simulations with direct differentiable access to coupled multiphysics operators. Instead of perturbing parameters externally and repeatedly solving the PDE system, the framework computes exact local differential sensitivities directly through reverse-mode automatic differentiation. 

All electrochemical operators, constitutive equations, and discretized PDE terms are represented as differentiable tensor computations in PyTorch. The discretized battery model can be written implicitly as $R(\mathbf{y}, \mathbf{p})=0$, where $\mathbf{y}$ is the coupled state vector and $\mathbf{p}$ is the parameter vector.

Differentiating with respect to a parameter $p_i$ yields $\frac{\partial \mathbf{y}}{\partial p_i} = - \left( \frac{\partial R}{\partial \mathbf{y}} \right)^{-1} \frac{\partial R}{\partial p_i}$. Instead of explicitly forming these Jacobians analytically or approximating them numerically using perturbations, automatic differentiation computes the derivative chain automatically through the computational graph. 

For a scalar output quantity $L(\mathbf{y},\mathbf{p})$ such as temperature rise rate ($\dot{T}$) or SEI degradation rate, the framework computes $\frac{\mathrm{d}L}{\mathrm{d}p_i}$ for all parameters simultaneously through a single backward pass. This eliminates truncation error, perturbation-size dependence, numerical cancellation error, and the need for repeated nonlinear solves. This capability is valuable for parameter importance analysis, gradient-based parameter estimation, pack heterogeneity studies, and degradation pathway analysis.
...
\begin{enumerate}
  \item \textbf{GPU Acceleration and Fast Solvers:} No open battery simulation framework currently leverages GPU vectorized automatic differentiation (like PyTorch \texttt{vmap} and \texttt{jacrev}) to handle the exact mathematics and matrix operations required for fast Jacobian computation at the pack level.
  \item \textbf{Pack Scalability:} DFN-resolution pack simulation has not been demonstrated beyond $\sim 20$ cells in an open framework without severe simplifications. Literature indicates that scaling above 20 cells is computationally difficult, with simulation times increasing exponentially using traditional CPU-based iterative methods.
  \item \textbf{Deeply Coupled Heterogeneity:} While studies explore specific phenomena (e.g., only thermal balancing, or only SEI heterogeneity), no framework natively couples full DFN electrochemical kinetics, dynamic thermal models, and differential SEI growth in large, arbitrary pack topologies simultaneously.
  \item \textbf{Plug-and-Play Modularity:} Existing tools rigidly couple physics to the solver. There is a lack of frameworks where different control strategies, thermal models, and SEI models (all referenced from established literature) can be modularly swapped to demonstrate capability without touching core code.
  \item \textbf{Nonlinear pack coupling:} Existing circuit-based frameworks (e.g.\ liionpack~\cite{Tranter2022}) linearise the cell as a voltage source with a fixed internal resistance for each circuit solve. The Butler--Volmer exchange current $I_0$ does not feed back into the current distribution at the same timestep, introducing $O(\Delta t)$ kinetic error.
  \item \textbf{Modular discretisation:} While a secondary priority compared to pack scaling, no framework offers runtime-interchangeable FDM/FVM/spectral discretisation on the same PDE system.
\end{enumerate}
