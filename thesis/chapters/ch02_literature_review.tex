...
Prior work on electrochemical battery sensitivity analysis has primarily relied on perturbation-based or repeated-simulation approaches because DFN-type electrochemical models are computationally expensive and strongly nonlinear. Several studies perform sensitivity analysis by repeatedly perturbing model parameters and re-running the electrochemical solver to observe changes in outputs such as voltage response, thermal behavior, or parameter identifiability~\cite{Forman2012}.

For example, approaches that estimate parameters of electrochemistry-based lithium-ion battery models using a two-step procedure typically perform sensitivity analysis of DFN parameters to identify which electrochemical parameters can be reliably estimated from current-voltage measurements~\cite{Marcicki2011}. Their approach evaluates parameter importance through repeated DFN simulations under perturbed parameter sets.

Similarly, parameter estimation studies of the Doyle--Fuller--Newman model often use parameter normalization, grouping, and sensitivity analysis to reduce parameter dimensionality and improve parameter estimation robustness~\cite{Kruger2001}. However, the methodology still relies on repeated forward DFN evaluations for parameter perturbations.
