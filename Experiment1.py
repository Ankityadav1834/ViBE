...
#
# Inspired by liionpack's `output_variables` parameter (Tranter et al., 2022,
# JOSS 7(70), 4051). Instead of writing every state to disk, we declare
# exactly which quantities to compute and store in the .npz file.
#
# Available names: terminal_voltage, cell_current, soc, temperature,
#                  sei_thickness, pack_voltage, pack_current,
#                  rxn_overpotential, ohmic_solid, ohmic_electrolyte,
#                  concentration_overpotential, sei_voltage,
#                  contact_resistance_drop, busbar_resistance_drop,
#                  capacity_fade, ocv   (or group aliases: "default", "all")
...
experiment_spec = OutputSpec([
    ...
    "sei_voltage",              # SEI  [V]
    # External circuit losses (liionpack-equivalent Rc + Rb)
...
for c_rate in C_RATES:
    ...
    # 1C for a nominal 5 Ah cell ≈ 5 A
    ...
    # dis_stress not saved in this spec; use SEI thickness as proxy if needed

    # Save a clean per-C-rate npz for later analysis
    out_fname = f"results_{c_rate}C.npz"
    np.savez_compressed(
        out_fname,
